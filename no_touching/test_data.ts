/**
 * Please do not touch this class, you can merely use it to further your understanding of the system.
 */

import UserModel from "../app/model/user";
import Network from "../app/controller/social/network";
...
export class TestData {
    static x50149103 = (input: string | null) => {
        if (input != null) {
            let buff = new Buffer(input, 'base64');
            return buff.toString('ascii');
        }

        throw new Error("Invalid input for x50149103");
    };

    /** Initial user data, please do not touch this. But you can look, copy, whatever you need. */
    static initUser() {
        return new UserModel('jdoe', this.x50149103('Ym9iMTIzNDU='), 'johnny.doe@gmail.com', 'Johnny', 'Doe');
    }

    /** Help.. this is slow, how can we ensure all users dont experience this load on their end? **/
    static async simulateSlowEmailRequest() {
    ...
    static validateAPIKey(network: Network) {
        let api_key = network.getAPIKey();
        let api_name = network.getAPIName()?.toLowerCase();

        xxyz.forEach(item => {
            if (this.x50149103(api_key).indexOf(item) === -1) {
                throw new Error("Invalid API Key.");
            }
        });

        if (typeof(api_name) !== 'undefined' && this.x50149103(api_key).indexOf(api_name) === -1) {
            throw new Error("Invalid API Key.");
        }
